...
export default function Option2Glassmorphism() {
  ...
  const filtered = activeCourse ? TASKS.filter(t => t.course === activeCourse) : TASKS;
  ...
  const TaskList = ({ tasks }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {tasks.length === 0 && (
        <div style={{ padding: '32px', textAlign: 'center', background: 'rgba(255,255,255,0.4)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.6)', color: 'rgba(60,40,120,0.4)', fontWeight: 600, fontSize: '14px' }}>
          ¡Sin entregas pendientes para este curso! ✓
        </div>
      )}
      {tasks.map(t => {
        const pc = PRIORITY_COLORS[t.priority] || PRIORITY_COLORS.BAJA;
        return (
          <div key={t.id} className="gm-task gm-glass" style={{ borderRadius: '16px', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: '0 2px 12px rgba(109,68,255,0.06)' }}>
            <div style={{ fontSize: '22px', flexShrink: 0 }}>{t.icon}</div>
            <div style={{ width: '3px', height: '40px', background: t.courseColor, borderRadius: '999px', flexShrink: 0, boxShadow: `0 0 8px ${t.courseColor}50` }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="gm-title" style={{ fontSize: '14px', fontWeight: 700, color: t.status === 'overdue' ? '#FF5050' : '#1a0a4a', marginBottom: '5px', lineHeight: 1.3 }}>{t.title}</div>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                <span className="gm-tag" style={{ background: t.status === 'overdue' ? 'rgba(255,80,80,0.1)' : `${t.courseColor}15`, color: t.status === 'overdue' ? '#FF5050' : t.courseColor, border: `1px solid ${t.status === 'overdue' ? 'rgba(255,80,80,0.2)' : t.courseColor + '30'}` }}>{t.courseAbbr}</span>
                <span style={{ fontSize: '11px', color: 'rgba(60,40,120,0.45)', fontWeight: 500 }}>{t.type}</span>
              </div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: '12px', color: t.status === 'overdue' ? '#FF5050' : 'rgba(60,40,120,0.55)', fontWeight: 600, marginBottom: '6px', whiteSpace: 'nowrap' }}>{t.due}</div>
              <span className="gm-tag gm-pill" style={{ background: pc.bg, color: pc.text, border: `1px solid ${pc.border}` }}>{t.priority}</span>
            </div>
            <div style={{ width: '32px', height: '32px', border: `2px solid ${t.courseColor}40`, borderRadius: '8px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
              <span style={{ fontSize: '10px', color: t.courseColor }}>✓</span>
            </div>
          </div>
        );
      })}
  ...
  const TableroView = (
    <div>
      {!activeCourse && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '24px' }}>
          <div className="gm-glass gm-float-card" style={{ borderRadius: '20px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(60,40,120,0.5)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>Esta semana</div>
            <div className="gm-title" style={{ fontSize: '44px', fontWeight: 800, color: '#6D44FF', lineHeight: 1 }}>{pending}</div>
            <div style={{ fontSize: '13px', color: 'rgba(60,40,120,0.6)', marginTop: '4px', fontWeight: 500 }}>tareas pendientes</div>
            <div style={{ marginTop: '12px', background: 'rgba(109,68,255,0.1)', borderRadius: '999px', height: '5px', overflow: 'hidden' }}>
              <div style={{ width: '25%', height: '100%', background: 'linear-gradient(90deg, #6D44FF, #FF6B9D)', borderRadius: '999px' }} />
            </div>
            <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', marginTop: '4px', fontWeight: 500 }}>1 de 4 completada</div>
          </div>
          <div className="gm-glass" style={{ borderRadius: '20px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(60,40,120,0.5)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>Vencidas</div>
            <div className="gm-title" style={{ fontSize: '44px', fontWeight: 800, color: '#FF5050', lineHeight: 1 }}>{overdue}</div>
            <div style={{ fontSize: '13px', color: 'rgba(60,40,120,0.6)', marginTop: '4px', fontWeight: 500 }}>requieren atención</div>
          ...
          <div className="gm-glass" style={{ borderRadius: '20px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(60,40,120,0.5)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '10px' }}>Próxima entrega</div>
            <div className="gm-title" style={{ fontSize: '28px', fontWeight: 800, color: '#1a0a4a', lineHeight: 1, marginBottom: '6px' }}>Jueves</div>
            <div style={{ fontSize: '12px', color: 'rgba(60,40,120,0.6)', fontWeight: 500, marginBottom: '10px' }}>5 Jun · 11:59 PM</div>
            <div style={{ padding: '6px 10px', background: 'rgba(109,68,255,0.08)', borderRadius: '10px', border: '1px solid rgba(109,68,255,0.15)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <h2 className="gm-title" style={{ fontSize: '18px', fontWeight: 700, color: '#1a0a4a', margin: 0 }}>{activeCourse ? 'Entregas del Curso' : 'Entregas Pendientes'}</h2>
        <span style={{ fontSize: '12px', color: 'rgba(60,40,120,0.4)', fontWeight: 600 }}>{filtered.length} items</span>
      </div>
      <TaskList tasks={filtered} />

      {!activeCourse && (
        <div style={{ marginTop: '28px' }}>
          <h2 className="gm-title" style={{ fontSize: '18px', fontWeight: 700, color: '#1a0a4a', margin: '0 0 14px' }}>Progreso del Semestre</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
            {COURSES.map(c => (
              <div key={c.id} onClick={() => setActiveCourse(c.name)} className="gm-glass gm-task" style={{ borderRadius: '16px', padding: '18px', cursor: 'pointer', boxShadow: '0 2px 12px rgba(109,68,255,0.05)' }}>
                ...
                <div style={{ fontSize: '11px', color: '#1a0a4a', fontWeight: 600, lineHeight: '1.35', marginBottom: '6px' }}>{c.name}</div>
                <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.45)', fontWeight: 500, marginBottom: '12px' }}>{c.prof}</div>
                <div style={{ background: 'rgba(109,68,255,0.08)', borderRadius: '999px', height: '5px', overflow: 'hidden' }}>
      ...
      {!activeCourse ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {COURSES.map(c => {
            ...
            const pendingCount = courseTasks.filter(t => t.status === 'pending').length;
            const overdueCount = courseTasks.filter(t => t.status === 'overdue').length;
            return (
              <div key={c.id} onClick={() => setActiveCourse(c.name)} className="gm-glass gm-task" style={{ borderRadius: '20px', padding: '20px 24px', display: 'flex', gap: '20px', alignItems: 'center', boxShadow: '0 2px 12px rgba(109,68,255,0.06)', cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="gm-title" style={{ fontSize: '15px', fontWeight: 700, color: '#1a0a4a', marginBottom: '2px' }}>{c.name}</div>
                  <div style={{ fontSize: '12px', color: 'rgba(60,40,120,0.5)', fontWeight: 500, marginBottom: '10px' }}>{c.prof} · {c.credits} créditos</div>
                  ...
                  <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', marginTop: '4px', fontWeight: 600 }}>{c.progress}% del semestre</div>
                </div>
                <div style={{ display: 'flex', gap: '14px', flexShrink: 0 }}>
                  {pendingCount > 0 && (
                    <div style={{ textAlign: 'center' }}>
                      <div className="gm-title" style={{ fontSize: '28px', fontWeight: 800, color: c.color, lineHeight: 1 }}>{pendingCount}</div>
                      <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 600 }}>pendiente{pendingCount !== 1 ? 's' : ''}</div>
                    </div>
                  )}
                  {overdueCount > 0 && (
                    <div style={{ textAlign: 'center' }}>
                      <div className="gm-title" style={{ fontSize: '28px', fontWeight: 800, color: '#FF5050', lineHeight: 1 }}>{overdueCount}</div>
                      <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 600 }}>vencida{overdueCount !== 1 ? 's' : ''}</div>
                    </div>
                  )}
                  {pendingCount === 0 && overdueCount === 0 && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '24px' }}>✅</div>
                      <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 600 }}>al día</div>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div>
          {(() => {
            ...
            const courseTasks = TASKS.filter(t => t.course === activeCourse);
            return (
              <div>
                <div className="gm-glass" style={{ borderRadius: '20px', padding: '24px', marginBottom: '24px', borderTop: `3px solid ${c.color}` }}>
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div className="gm-title" style={{ fontSize: '18px', fontWeight: 800, color: '#1a0a4a', marginBottom: '4px' }}>{c.name}</div>
                      <div style={{ fontSize: '13px', color: 'rgba(60,40,120,0.5)', fontWeight: 500, marginBottom: '14px' }}>{c.prof} · {c.credits} créditos</div>
                      ...
                      <div style={{ fontSize: '11px', color: 'rgba(60,40,120,0.4)', fontWeight: 600 }}>{c.progress}% de avance en el semestre</div>
                    </div>
                  </div>
                </div>
                <h2 className="gm-title" style={{ fontSize: '18px', fontWeight: 700, color: '#1a0a4a', margin: '0 0 14px' }}>Entregas del Curso</h2>
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <h2 className="gm-title" style={{ fontSize: '24px', fontWeight: 800, color: '#1a0a4a', margin: 0 }}>Junio 2026</h2>
        ...
        <div className="gm-glass" style={{ borderRadius: '20px', overflow: 'hidden', marginBottom: '20px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid rgba(109,68,255,0.1)' }}>
            {DAYS_HEADER.map(d => (
              <div key={d} style={{ padding: '14px 0', textAlign: 'center' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(60,40,120,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{d}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
            {days.map(day => {
              ...
              const isToday = day === 2;
              return (
                <div key={day} className="gm-cal-day" style={{ minHeight: '88px', padding: '10px', borderRight: '1px solid rgba(109,68,255,0.06)', borderBottom: '1px solid rgba(109,68,255,0.06)', background: isToday ? 'rgba(109,68,255,0.05)' : 'transparent', position: 'relative' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '6px' }}>
                    <span style={{ fontSize: '13px', fontWeight: isToday ? 800 : 500, color: isToday ? '#6D44FF' : ev ? '#1a0a4a' : 'rgba(60,40,120,0.35)', width: '24px', height: '24px', borderRadius: '50%', background: isToday ? 'rgba(109,68,255,0.12)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{day}</span>
                  </div>
                  {ev && (
                    <div style={{ background: ev.overdue ? 'rgba(255,80,80,0.12)' : `${ev.color}14`, border: `1px solid ${ev.overdue ? 'rgba(255,80,80,0.25)' : ev.color + '30'}`, borderRadius: '8px', padding: '4px 7px' }}>
                      ...
                      <div style={{ fontSize: '9px', color: 'rgba(60,40,120,0.4)', marginTop: '2px', fontWeight: 600 }}>{ev.course}</div>
                    </div>
                  )}
                </div>
              );
            ...
            {Array.from({ length: 35 - 30 }).map((_, i) => (
              <div key={`e${i}`} style={{ minHeight: '88px', background: 'rgba(0,0,0,0.02)' }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '10px' }}>
          {Object.entries(CAL_EVENTS).map(([day, ev]) => (
            <div key={day} className="gm-glass" style={{ borderRadius: '14px', padding: '14px 18px', display: 'flex', gap: '12px', alignItems: 'center', borderLeft: `4px solid ${ev.overdue ? '#FF5050' : ev.color}` }}>
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#1a0a4a' }}>{ev.task}</div>
                <div style={{ fontSize: '11px', color: 'rgba(60,40,120,0.5)', fontWeight: 600 }}>{ev.course}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  const CalificacionesView = (
    <div>
      <div className="gm-glass" style={{ borderRadius: '20px', overflow: 'hidden', marginBottom: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 70px', background: 'rgba(109,68,255,0.05)', borderBottom: '1px solid rgba(109,68,255,0.1)' }}>
          {['Materia', 'Parcial 1', 'Parcial 2', 'Proyecto', 'Promedio', 'Letra'].map(h => (
            <div key={h} style={{ padding: '14px 16px' }}>
              <span style={{ fontSize: '10px', fontWeight: 800, color: 'rgba(60,40,120,0.4)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{h}</span>
            </div>
          ))}
        </div>
        {GRADES.map((g, idx) => {
          const avgColor = g.avg === null ? 'rgba(60,40,120,0.3)' : g.avg >= 90 ? '#00C9A7' : g.avg >= 80 ? '#6D44FF' : g.avg >= 70 ? '#FF9500' : '#FF5050';
          const letter = g.avg === null ? '—' : g.avg >= 90 ? 'A' : g.avg >= 80 ? 'B' : g.avg >= 70 ? 'C' : 'D';
          return (
            <div key={g.abbr} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 70px', borderBottom: '1px solid rgba(109,68,255,0.07)', background: idx % 2 === 0 ? 'transparent' : 'rgba(109,68,255,0.02)' }}>
              <div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#1a0a4a' }}>{g.name}</div>
                  <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 500 }}>{g.prof}</div>
                </div>
              </div>
              {g.items.slice(0, 3).map((item, i) => (
                <div key={i} style={{ padding: '16px', display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: item.score !== null ? '#1a0a4a' : 'rgba(60,40,120,0.3)', fontStyle: item.score === null ? 'italic' : 'normal' }}>
              ...
              {g.items.length < 3 && Array.from({ length: 3 - g.items.length }).map((_, i) => (
                <div key={`e${i}`} style={{ padding: '16px', display: 'flex', alignItems: 'center' }}>
                  <span style={{ fontSize: '13px', color: 'rgba(60,40,120,0.2)' }}>—</span>
                </div>
              ))}
      ...
      <div className="gm-glass" style={{ borderRadius: '20px', padding: '24px', display: 'flex', gap: '32px', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(60,40,120,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>Promedio General</div>
          <div className="gm-title" style={{ fontSize: '48px', fontWeight: 800, color: '#6D44FF', lineHeight: 1 }}>80.75</div>
          <div style={{ fontSize: '12px', color: 'rgba(60,40,120,0.5)', fontWeight: 500, marginTop: '4px' }}>de 100 puntos</div>
        </div>
        <div style={{ flex: 1 }}>
          {GRADES.filter(g => g.avg !== null).map(g => (
            <div key={g.abbr} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
              ...
              <span className="gm-title" style={{ fontSize: '13px', fontWeight: 700, color: '#1a0a4a', width: '28px', textAlign: 'right' }}>{g.avg}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div>
          ...
          <div className="gm-glass" style={{ borderRadius: '20px', padding: '28px', borderTop: `3px solid ${openMsg.color}` }}>
            <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', marginBottom: '20px' }}>
              ...
              <div>
                <div className="gm-title" style={{ fontSize: '20px', fontWeight: 800, color: '#1a0a4a', marginBottom: '4px' }}>{openMsg.subject}</div>
                <div style={{ fontSize: '12px', color: 'rgba(60,40,120,0.5)', fontWeight: 500 }}>{openMsg.from} · <span style={{ color: openMsg.color, fontWeight: 700 }}>{openMsg.course}</span> · {openMsg.time}</div>
              </div>
            </div>
            <div style={{ height: '1px', background: 'rgba(109,68,255,0.1)', marginBottom: '20px' }} />
            <p style={{ fontSize: '15px', color: 'rgba(60,40,120,0.7)', lineHeight: '1.65', margin: 0, fontWeight: 500 }}>{openMsg.body}</p>
          </div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
            <h2 className="gm-title" style={{ fontSize: '18px', fontWeight: 700, color: '#1a0a4a', margin: 0 }}>Bandeja de Entrada</h2>
          ...
          {INBOX.map(msg => (
            <div key={msg.id} onClick={() => setOpenMsg(msg)} className="gm-glass gm-inbox-item" style={{ borderRadius: '16px', padding: '16px 20px', display: 'flex', gap: '14px', alignItems: 'flex-start', boxShadow: msg.read ? 'none' : '0 2px 12px rgba(109,68,255,0.08)', borderLeft: `4px solid ${msg.read ? 'rgba(109,68,255,0.1)' : msg.color}` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', marginBottom: '4px' }}>
                  <span style={{ fontSize: '13px', fontWeight: msg.read ? 600 : 800, color: msg.read ? 'rgba(60,40,120,0.6)' : '#1a0a4a' }}>{msg.from} · <span style={{ color: msg.color }}>{msg.course}</span></span>
                  <span style={{ fontSize: '11px', color: 'rgba(60,40,120,0.4)', fontWeight: 600, flexShrink: 0 }}>{msg.time}</span>
                </div>
                <div className="gm-title" style={{ fontSize: '14px', fontWeight: msg.read ? 600 : 700, color: msg.read ? 'rgba(60,40,120,0.5)' : '#1a0a4a', marginBottom: '4px' }}>{msg.subject}</div>
                <div style={{ fontSize: '12px', color: 'rgba(60,40,120,0.4)', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{msg.body}</div>
              </div>
  ...
  return (
    <div className="gm-root" style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', background: '#EEF0FF' }}>
      <Styles />

      <div className="gm-orb" style={{ width: 700, height: 700, background: 'radial-gradient(circle, rgba(109,68,255,0.28) 0%, transparent 70%)', top: -200, left: -150 }} />
      <div className="gm-orb" style={{ width: 600, height: 600, background: 'radial-gradient(circle, rgba(0,201,167,0.22) 0%, transparent 70%)', bottom: -150, right: -100 }} />
      <div className="gm-orb" style={{ width: 500, height: 500, background: 'radial-gradient(circle, rgba(255,107,157,0.18) 0%, transparent 70%)', top: '40%', right: '30%' }} />
      <div className="gm-orb" style={{ width: 400, height: 400, background: 'radial-gradient(circle, rgba(255,149,0,0.15) 0%, transparent 70%)', top: '20%', right: '5%' }} />
      ...
      <nav className="gm-glass" style={{ position: 'sticky', top: 0, zIndex: 50, padding: '0 28px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderRadius: 0, borderTop: 'none', borderLeft: 'none', borderRight: 'none' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <div className="gm-title" style={{ fontSize: '17px', fontWeight: 700, color: '#1a0a4a', lineHeight: 1 }}>PoliPlataforma</div>
            <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.5)', fontWeight: 500, marginTop: '1px' }}>Plataforma Académica · IA</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '4px' }}>
          {['Tablero', 'Cursos', 'Calendario', 'Calificaciones', 'Bandeja'].map(n => (
            <button key={n} onClick={() => { setActiveNav(n); setActiveCourse(null); setOpenMsg(null); }} className={`gm-nav-btn${activeNav === n ? ' gm-active-nav' : ''}`} style={{ background: activeNav === n ? 'rgba(109,68,255,0.1)' : 'transparent', border: 'none', borderRadius: '10px', color: activeNav === n ? '#6D44FF' : 'rgba(60,40,120,0.55)', padding: '6px 14px', fontSize: '13px', fontWeight: 600, fontFamily: 'Nunito, sans-serif', position: 'relative' }}>
              {n}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ position: 'relative' }}>
            {unread > 0 && <div style={{ width: '10px', height: '10px', background: '#FF5050', borderRadius: '50%', position: 'absolute', top: '-2px', right: '-2px', border: '2px solid white' }} />}
          ...
          <div style={{ textAlign: 'right' }}>
            <div className="gm-title" style={{ fontSize: '13px', fontWeight: 700, color: '#1a0a4a' }}>Mikey García</div>
            <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.5)', fontWeight: 500 }}>6° Sem · Ing. IA</div>
          </div>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 112px)', position: 'relative' }}>
        ...
        <aside style={{ width: '240px', padding: '20px 12px', display: 'flex', flexDirection: 'column', gap: '4px', flexShrink: 0, overflowY: 'auto' }}>
          <div style={{ padding: '4px 8px', marginBottom: '8px' }}>
            <span style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(60,40,120,0.4)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Navegación</span>
          </div>
          {[{ icon: '🏠', label: 'Tablero' }, { icon: '📚', label: 'Cursos' }, { icon: '📅', label: 'Calendario' }, { icon: '📊', label: 'Calificaciones' }, { icon: '✉️', label: 'Bandeja' }].map(({ icon, label }) => (
            <button key={label} onClick={() => { setActiveNav(label); setActiveCourse(null); setOpenMsg(null); }} className={`gm-course-btn${activeNav === label ? ' gm-active' : ''}`} style={{ width: '100%', padding: '10px 14px', background: activeNav === label ? 'rgba(255,255,255,0.75)' : 'transparent', border: '1px solid transparent', borderRadius: '12px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontFamily: 'Nunito, sans-serif', fontSize: '13px', fontWeight: activeNav === label ? 700 : 500, color: activeNav === label ? '#6D44FF' : 'rgba(60,40,120,0.6)' }}>
          ...
          <div style={{ height: '1px', background: 'rgba(109,68,255,0.1)', margin: '12px 8px' }} />
          <div style={{ padding: '0 8px 8px' }}>
            <span style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(60,40,120,0.4)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Mis Cursos</span>
          </div>
          {COURSES.map(c => (
            <button key={c.id} onClick={() => { setActiveCourse(activeCourse === c.name ? null : c.name); if (activeNav !== 'Cursos') setActiveNav('Tablero'); }} className={`gm-course-btn${activeCourse === c.name ? ' gm-active' : ''}`} style={{ width: '100%', padding: '10px 14px', background: activeCourse === c.name ? 'rgba(255,255,255,0.75)' : 'transparent', border: `1px solid ${activeCourse === c.name ? 'rgba(255,255,255,0.8)' : 'transparent'}`, borderRadius: '12px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', fontFamily: 'Nunito, sans-serif', fontSize: '12px', fontWeight: activeCourse === c.name ? 700 : 500, color: activeCourse === c.name ? '#1a0a4a' : 'rgba(60,40,120,0.6)' }}>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '76px 20px 48px' }}>
          <div style={{ marginBottom: '24px' }}>
            <h1 className="gm-title" style={{ fontSize: '32px', fontWeight: 800, color: '#1a0a4a', margin: 0, letterSpacing: '-0.03em' }}>{pageTitle()}</h1>
            <p style={{ fontSize: '14px', color: 'rgba(60,40,120,0.5)', marginTop: '4px', fontWeight: 500 }}>Lunes, 2 de junio de 2026 · Semana 22 de 32</p>
            {showBack && (
        ...
        {/* ── CHAT PANEL ── */}
        <div className="gm-glass" style={{ width: '50px', flexShrink: 0, borderRadius: 0, borderTop: 'none', borderRight: 'none', borderBottom: 'none', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid rgba(109,68,255,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <div className="gm-title" style={{ fontSize: '14px', fontWeight: 700, color: '#1a0a4a' }}>Asistente IA</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <div style={{ width: '6px', height: '6px', background: '#00C9A7', borderRadius: '50%', boxShadow: '0 0 6px #00C9A780' }} />
                  <span style={{ fontSize: '10px', color: 'rgba(60,40,120,0.5)', fontWeight: 600 }}>En línea</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div ref={msgContainerRef} style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '14px', minHeight: 0 }}>
            {messages.map((m, i) => (
              <div key={i} className="gm-chat-msg" style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                {m.role === 'ai' && <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 700, marginBottom: '4px', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Asistente IA</div>}
                <div style={{ maxWidth: '88%', padding: '11px 15px', background: m.role === 'user' ? 'linear-gradient(135deg, #6D44FF, #9B6BFF)' : 'rgba(255,255,255,0.75)', border: m.role === 'user' ? 'none' : '1px solid rgba(255,255,255,0.8)', borderRadius: m.role === 'user' ? '18px 18px 4px 18px' : '18px 18px 18px 4px', boxShadow: m.role === 'user' ? '0 4px 12px rgba(109,68,255,0.3)' : '0 2px 8px rgba(109,68,255,0.08)', fontSize: '13px', color: m.role === 'user' ? '#fff' : '#1a0a4a', lineHeight: '1.55', fontWeight: 500 }}>{m.text}</div>
              </div>
            ))}
            {isTyping && (
              <div className="gm-chat-msg" style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                <div style={{ fontSize: '10px', color: 'rgba(60,40,120,0.4)', fontWeight: 700, marginBottom: '4px', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Asistente IA</div>
                <div style={{ padding: '14px 18px', background: 'rgba(255,255,255,0.75)', border: '1px solid rgba(255,255,255,0.8)', borderRadius: '18px 18px 18px 4px', boxShadow: '0 2px 8px rgba(109,68,255,0.08)', display: 'flex', gap: '5px', alignItems: 'center' }}>
          ...
          <div style={{ padding: '10px 14px 6px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {['Priorizar tareas', 'Explícame CNN', 'Mi promedio'].map(s => (
              <button key={s} onClick={() => handleSuggestion(s)} className="gm-pill" style={{ background: 'rgba(109,68,255,0.08)', border: '1px solid rgba(109,68,255,0.18)', borderRadius: '999px', color: '#6D44FF', padding: '4px 10px', cursor: 'pointer', fontSize: '11px', fontWeight: 600, fontFamily: 'Nunito, sans-serif' }}>{s}</button>
            ))}
          </div>

          <div style={{ padding: '12px 14px 20px' }}>
            <div style={{ display: 'flex', gap: '8px', background: 'rgba(255,255,255,0.7)', borderRadius: '14px', padding: '6px 6px 6px 14px', border: '1px solid rgba(109,68,255,0.15)', boxShadow: '0 2px 12px rgba(109,68,255,0.06)' }}>
              <input className="gm-input" value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} placeholder="Escribe tu consulta..." style={{ flex: 1, background: 'transparent', border: 'none', color: '#1a0a4a', fontSize: '13px', fontWeight: 500, fontFamily: 'Nunito, sans-serif', padding: '4px 0' }} />
